...
class SimpleCenterlineGraph(networkx.classes.graph.Graph):
    ...
    def __init__(self, **attr):
        ...
        Parameters
        ----------
        attr : **dict or key=value pairs. Mandatory attributes are:
            image_id : str
    ...
    def get_relative_coronary_ostia_node_id(self, node_id: str) -> tuple[str] | tuple[str, str]:
        ...
        if node['arterial_tree'].value != ArteryPointTree.RL.value:
            ...
            for n in self.nodes:
                if self.nodes[n]['arterial_tree'].value == node['arterial_tree'].value and self.nodes[n]['topology_class'].value == ArteryPointTopologyClass.OSTIUM.value:
                    return tuple([n])
        else:
            ...
            for n in self.nodes:
                if self.nodes[n]['topology_class'].value == ArteryPointTopologyClass.OSTIUM.value:
                    if self.nodes[n]['arterial_tree'].value == ArteryPointTree.LEFT.value:
                        left_ostium_n = n
                    elif self.nodes[n]['arterial_tree'].value == ArteryPointTree.RIGHT.value:
                        right_ostium_n = n
                    else:
    ...
    def convert_to_3dslicer_opencurve(self, save_directory: str, affine_transformation_matrix: numpy.ndarray | None = None) -> None:
        ...
        for n in self.nodes:
            if self.nodes[n]['topology_class'] == ArteryPointTopologyClass.ENDPOINT:
                endpoint_node_id = n
                # get coronary ostium node id that is connected to this endpoint
                ostia_node_id = self.get_relative_coronary_ostia_node_id(endpoint_node_id)
                for ostium_node_id in ostia_node_id:
                    ...
                    path = networkx.algorithms.shortest_path(self, ostium_node_id, endpoint_node_id)
                    # create the 3D Slicer open curve file content in json format
                    arr_ = numpy.array(
                        [[self.nodes[n]['x'], self.nodes[n]['y'], self.nodes[n]['z']] for n in path]
                    )
                    ...
                    descriptions_ = [f"{self.nodes[n]['arterial_tree'].name} {self.nodes[n]['topology_class'].name}" for n in path]
                    ...
                    # create the file
                    if self.nodes[ostium_node_id]['arterial_tree'] == ArteryPointTree.LEFT:
                        tree = "left"
                    if self.nodes[ostium_node_id]['arterial_tree'] == ArteryPointTree.RIGHT:
                        tree = "right"
    ...
    def convert_to_3dslicer_fiducials(self, save_filename: str, affine_transformation_matrix: numpy.ndarray | None = None) -> None:
        ...
        arr_ = numpy.array(
            [[self.nodes[n]['x'], self.nodes[n]['y'], self.nodes[n]['z']] for n in self.nodes]
        )
        # - transform the points according to the transformation matrix
        arr_ = numpy.concatenate((arr_, numpy.ones((arr_.shape[0], 1))), axis=1).T # 4 x N
        arr_ = numpy.matmul(affine_transformation_matrix, arr_).T
        arr_ = arr_[:, :3]
        # - get other data
        labels_ = [n for n in self.nodes]
        descriptions_ = [f"{self.nodes[n]['arterial_tree'].name} {self.nodes[n]['topology_class'].name}" for n in self.nodes]
    ...
    def load(filename: str) -> SimpleCenterlineGraph:
        ...
        Relies on the function ``load_graph``.
        
        Parameters
        ----------
        filename : str
            The name of the file to load the graph from.
