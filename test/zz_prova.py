...
if __name__ == "__main__":
    ...
    for i, vertex in enumerate(v):
        node_dict = node.getNodeDictFromKeyList( node.VertexNodeFeatures_KeysList )
        node.set_VertexNodeFeaturesVertex(node_dict, vertex)
        ...
        if i != 0:
            edge_dict = edge.getEdgeDictFromKeyList(edge.SimpleCenterlineEdgeFeatures_KeysList)
